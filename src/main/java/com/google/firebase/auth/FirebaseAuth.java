...
public class FirebaseAuth {
   ...
   * @return A {@link ApiFuture} which will complete successfully with the created Firebase
   *     Custom Token, or unsuccessfully with the failure Exception.
   */
  public ApiFuture<String> createCustomToken(final String uid) {
   ...
   *     Custom Token, or unsuccessfully with the failure Exception.
   */
  public ApiFuture<String> createCustomToken(
   ...
   * account)
   *
   * <p>If the token is valid, the returned {@link ApiFuture} will complete successfully and
   ...
   * @return A {@link ApiFuture} which will complete successfully with the parsed token, or
   *     unsuccessfully with the failure Exception.
   */
  public ApiFuture<FirebaseToken> verifyIdToken(final String token) {
   ...
   * @return A {@link ApiFuture} which will complete successfully with a {@link UserRecord}
   ...
   */
  public ApiFuture<UserRecord> getUser(final String uid) {
   ...
   */
  public ApiFuture<UserRecord> getUserByEmail(final String email) {
   ...
   */
  public ApiFuture<UserRecord> createUser(final CreateRequest request) {
   ...
   */
  public ApiFuture<UserRecord> updateUser(final UpdateRequest request) {
   ...
   *
   * @param uid A user ID string.
   * @return A {@link ApiFuture} which will complete successfully when the specified user
   *     account has been deleted. If an error occurs while deleting the user account, the future
   *     fails with a FirebaseAuthException.
   * @throws IllegalArgumentException If the user ID string is null or empty.
   */
  public ApiFuture<Void> deleteUser(final String uid) {
    checkArgument(!Strings.isNullOrEmpty(uid), "uid must not be null or empty");
  ...
  private <T> ApiFuture<T> schedule(Callable<T> command) {
